odoo.define('chatbot.legacy.chatbot.chatbot', function (require) {
    "use strict";
    ...
    var WebsiteChatbot = require('chatbot.legacy.chatbot.model.WebsiteChatbot');
    var WebsiteChatbotMessage = require('chatbot.legacy.chatbot.model.WebsiteChatbotMessage');
    var WebsiteChatbotWindow = require('chatbot.legacy.chatbot.WebsiteChatbotWindow');

    var _t = core._t;
    var QWeb = core.qweb;

// Constants
    var CHATBOT_COOKIE_HISTORY = 'chatbot_history';
    var HISTORY_LIMIT = 15;

// History tracking
    var page = window.location.href.replace(/^.*\/\/[^/]+/, '');
    var pageHistory = utils.get_cookie(CHATBOT_COOKIE_HISTORY);
    var urlHistory = [];
    if (pageHistory) {
    ...
    if (!_.contains(urlHistory, page)) {
        ...
        utils.set_cookie(CHATBOT_COOKIE_HISTORY, JSON.stringify(urlHistory), 60 * 60 * 24); // 1 day cookie
    }

    var ChatbotButton = Widget.extend({
        className: 'openerp o_chatbot_button d-print-none',
        custom_events: {
        ...
        async willStart() {
            var cookie = utils.get_cookie('chatbot_session');
            if (cookie) {
                var channel = JSON.parse(cookie);
                this._history = await session.rpc('/mail/chat_history', {uuid: channel.uuid, limit: 100});
                this._history.reverse().forEach(message => {
                    message.body = utils.Markup(message.body);
                });
            } else {
                const result = await session.rpc('/chatbot/init', {channel_id: this.options.channel_id});
        ...
        start: function () {
            ...
            if (this._history) {
                ...
                var autoPopupCookie = utils.get_cookie('chatbot_auto_popup');
            ...
            // If website_event_track installed, put the chatbot banner above the PWA banner.
        ...
        _addMessage: function (data, options) {
            ...
            var message = new WebsiteChatbotMessage(this, data, options);
            ...
            if (this._chatbot) {
                this._chatbot.addMessage(message);
            }
        ...
        _askFeedback: function () {
            ...
            var feedback = new Feedback(this, this._chatbot);
        ...
        _closeChat: function () {
            ...
            utils.set_cookie('chatbot_session', "", -1); // remove cookie
        },
        /**
         * @private
        ...
        _handleNotification: function ({payload, type}) {
            switch (type) {
                case 'chatbot.history_command': {
                    if (payload.id !== this._chatbot._id) {
                        return;
                    }
                    const cookie = utils.get_cookie(CHATBOT_COOKIE_HISTORY);
                    const history = cookie ? JSON.parse(cookie) : [];
                    session.rpc('/chatbot/history', {
                        pid: this._chatbot.getOperatorPID()[0],
                        channel_uuid: this._chatbot.getUUID(),
                        page_history: history,
                    });
                    return;
                }
                case 'mail.channel.partner/typing_status': {
                    if (payload.channel_id !== this._chatbot._id) {
                        return;
                    }
                    ...
                    if (payload.is_typing) {
                        this._chatbot.registerTyping({partnerID});
                    } else {
                        this._chatbot.unregisterTyping({partnerID});
                    }
                    return;
                }
                case 'mail.channel/new_message': {
                    ...
                    if (this._chatWindow.isFolded() || !this._chatWindow.isAtBottom()) {
                        this._chatbot.incrementUnreadCounter();
                    }
        ...
        _loadQWebTemplate: function () {
            return session.rpc('/chatbot/load_templates').then(function (templates) {
        ...
        _openChat: _.debounce(function () {
            ...
            if (cookie) {
                ...
                def = session.rpc('/chatbot/get_session', {
                    channel_id: this.options.channel_id,
                    anonymous_name: this.options.default_username,
                    previous_operator_id: this._get_previous_operator_id(),
                }, {shadow: true});
            }
            def.then(function (chatbotData) {
                if (!chatbotData || !chatbotData.operator_pid) {
                    try {
                    ...
                    self._chatbot = new WebsiteChatbot({
                        parent: self,
                        data: chatbotData
                    });
                    return self._openChatWindow().then(function () {
                        ...
                        self.call('bus_service', 'addChannel', self._chatbot.getUUID());
                        self.call('bus_service', 'startPolling');

                        utils.set_cookie('chatbot_session', utils.unaccent(JSON.stringify(self._chatbot.toData()), true), 60 * 60);
                        utils.set_cookie('chatbot_auto_popup', JSON.stringify(false), 60 * 60);
                        if (chatbotData.operator_pid[0]) {
                            // chatbotData.operator_pid contains a tuple (id, name)
                            // we are only interested in the id
                            var operatorPidId = chatbotData.operator_pid[0];
                            var oneWeek = 7 * 24 * 60 * 60;
                            utils.set_cookie('chatbot_previous_operator_pid', operatorPidId, oneWeek);
                        }
                    });
                }
            }).then(function () {
         ...
         *
         * The information is stored in the 'chatbot_previous_operator_pid' cookie.
         *
         * @private
         * @return {integer} operator_id.partner_id.id if the cookie is set
         */
        _get_previous_operator_id: function () {
            var cookie = utils.get_cookie('chatbot_previous_operator_pid');
            if (cookie) {
                return cookie;
            }
        ...
        _openChatWindow: function () {
            ...
            this._chatWindow = new WebsiteChatbotWindow(this, this._chatbot, options);
        ...
        _renderMessages: function () {
            ...
            this._chatbot.setMessages(this._messages);
        ...
        _sendMessage: function (message) {
            var self = this;
            this._chatbot._notifyMyselfTyping({typing: false});
            return session
                .rpc('/mail/chat_post', {uuid: this._chatbot.getUUID(), message_content: message.content})
        ...
        _sendWelcomeMessage: function () {
            if (this.options.default_message) {
                this._addMessage({
                    id: '_welcome',
                    attachment_ids: [],
                    author_id: this._chatbot.getOperatorPID(),
                    body: this.options.default_message,
                    date: time.datetime_to_str(new Date()),
                    model: "mail.channel",
                    res_id: this._chatbot.getID(),
                }, {prepend: true});
            }
        },
        ...
        _onSaveChatWindow: function (ev) {
            ev.stopPropagation();
            utils.set_cookie('chatbot_session', utils.unaccent(JSON.stringify(this._chatbot.toData()), true), 60 * 60);
        },
        /**
         * @private
         * @param {OdooEvent} ev
...
//    template: 'chatbot.legacy.chatbot.FeedBack',
//
//    events: {
//        'click .o_chatbot_rating_choices img': '_onClickSmiley',
//        'click .o_chatbot_no_feedback span': '_onClickNoFeedback',
//        'click .o_rating_submit_button': '_onClickSend',
//        'click .o_email_chat_button': '_onEmailChat',
//        'click .o_chatbot_email_error .alert-link': '_onTryAgain',
//    },
//
//    /**
//     * @param {?} parent
//     * @param {chatbot.legacy.chatbot.model.WebsiteChatbot} chatbot
//     */
//    init: function (parent, chatbot) {
//        this._super(parent);
...
//        var self = this;
//        var args = {
//            uuid: this._chatbot.getUUID(),
//            rate: this.rating,
//            reason: reason,
//        };
//        this.dp.add(session.rpc('/chatbot/feedback', args)).then(function (response) {
...
//        this.$('.o_chatbot_rating_box').empty().append($('<div />', {
...
//        this.$('.o_chatbot_rating_reason').hide();
...
//        this.$('.o_chatbot_rating_choices img').removeClass('selected');
//        this.$('.o_chatbot_rating_choices img[data-value="' + this.rating + '"]').addClass('selected');
//
//        // only display textearea if bad smiley selected
//        if (this.rating !== 5) {
//            this._sendFeedback();
//            this.$('.o_chatbot_rating_reason').show();
//        } else {
//            this.$('.o_chatbot_rating_reason').hide();
//            this._showThanksMessage();
//            this._sendFeedback();
...
//            this._rpc({
//                route: '/chatbot/email_chatbot_transcript',
//                params: {
//                    uuid: this._chatbot.getUUID(),
//                    email: $email.val(),
//                }
//            }).then(function () {
//                self.$('.o_chatbot_email').html($('<strong />', { text: _t('Conversation Sent') }));
//            }).guardedCatch(function () {
//                self.$('.o_chatbot_email').hide();
//                self.$('.o_chatbot_email_error').show();
//            });
//        } else {
...
//        this.$('.o_chatbot_email_error').hide();
//        this.$('.o_chatbot_email').show();
//    },
//});
...
//});
    ...
    odoo.define('chatbot.legacy.chatbot.model.WebsiteChatbot', function (require) {
        "use strict";

        var AbstractThread = require('chatbot.legacy.mail.model.AbstractThread');
        var ThreadTypingMixin = require('chatbot.legacy.mail.model.ThreadTypingMixin');

        var session = require('web.session');

        /**
         * Thread model that represents a chatbot on the website-side. This chatbot
         * is not linked to the mail service.
         */
        var WebsiteChatbot = AbstractThread.extend(ThreadTypingMixin, {
             ...
             * @param {boolean} [params.data.folded] states whether the chatbot is
             *   folded or not. It is considered only if this is defined and it is a
             *   boolean.
             * @param {integer} params.data.id the ID of this chatbot.
             * @param {integer} [params.data.message_unread_counter] the unread counter
             *   of this chatbot.
             * @param {Array} params.data.operator_pid
             * @param {string} params.data.name the name of this chatbot.
             * @param {string} [params.data.state] if 'folded', the chatbot is folded.
             *   This is ignored if `folded` is provided and is a boolean value.
             * @param {string} params.data.uuid the UUID of this chatbot.
             * @param {chatbot.legacy.chatbot.chatbot:ChatbotButton} params.parent
             */
            ...
            /**
             * @override
             * @returns {chatbot.legacy.chatbot.model.WebsiteChatbotMessage[]}
             */
            ...
            /**
             * Increments the unread counter of this chatbot by 1 unit.
             *
             * Note: this public method makes sense because the management of messages
             * for website chatbot is external. This method should be dropped when
             ...
             *
             * @param {chatbot.legacy.chatbot.model.WebsiteChatbotMessage[]} messages
             */
            ...
            _notifyMyselfTyping: function (params) {
                return session.rpc('/chatbot/notify_typing', {
                    uuid: this.getUUID(),
            ...
            /**
             * Warn that the unread counter has been updated on this chatbot
             *
             * @override
             * @private
             */
    ...
    odoo.define('chatbot.legacy.chatbot.model.WebsiteChatbotMessage', function (require) {
        "use strict";

        var AbstractMessage = require('chatbot.legacy.mail.model.AbstractMessage');

        /**
         * This is a message that is handled by chatbot, without making use of the
         * mail.Manager. The purpose of this is to make chatbot compatible with
         * mail.widget.Thread.
         *
         * @see chatbot.legacy.mail.model.AbstractMessage for more information.
         */
        var WebsiteChatbotMessage = AbstractMessage.extend({

            /**
             * @param {chatbot.legacy.chatbot.chatbot.ChatbotButton} parent
             * @param {Object} data
             * @param {Object} options
        ...
        return WebsiteChatbotMessage;

    });

    odoo.define('chatbot.legacy.chatbot.WebsiteChatbotWindow', function (require) {
        "use strict";

        var AbstractThreadWindow = require('chatbot.legacy.mail.AbstractThreadWindow');

        /**
         * This is the widget that represent windows of chatbot in the frontend.
         *
         * @see chatbot.legacy.mail.AbstractThreadWindow for more information
         */
        var ChatbotWindow = AbstractThreadWindow.extend({
             ...
             * @override
             * @param {chatbot.legacy.chatbot.chatbot:ChatbotButton} parent
             * @param {chatbot.legacy.chatbot.model.WebsiteChatbot} thread
             * @param {Object} [options={}]
            ...
            /**
             * Warn the parent widget (ChatbotButton)
             *
             * @override
    ...
    odoo.define('chatbot.legacy.mail.model.AbstractThread', function (require) {
        "use strict";
         ...
         * (which are compatible with mail service) or website chatbots.
         *
        ...
        var AbstractThread = Class.extend(Mixins.EventDispatcherMixin, {
             ...
             *
             * @param {chatbot.legacy.mail.model.AbstractMessage} message
             */
            ...
            /**
             * @abstract
             * @returns {chatbot.legacy.mail.model.AbstractMessage[]}
             */
            getMessages: function () {
            },
            /**
             ...
             * @see {chatbot.legacy.mail.model.ThreadSeenMixin} to enable this feature on a thread.
             *
             * @returns {boolean}
             */
    ...
    odoo.define('chatbot.legacy.mail.model.ThreadTypingMixin', function (require) {
        "use strict";

        var CCThrottleFunction = require('chatbot.legacy.mail.model.CCThrottleFunction');
        var Timer = require('chatbot.legacy.mail.model.Timer');
        var Timers = require('chatbot.legacy.mail.model.Timers');
    ...
    odoo.define('chatbot.legacy.mail.model.AbstractMessage', function (require) {
        "use strict";
         ...
         * The purpose of this interface is to make chatbot compatible with
         * mail.widget.Thread, as this widget was designed to work with messages that
         * are instances of mail.model.Messages.
         *
         * Ideally, chatbot should also handle mail.model.Message, but this is not
    ...
    odoo.define('chatbot.legacy.mail.AbstractThreadWindow', function (require) {
        "use strict";

        var ThreadWidget = require('chatbot.legacy.mail.widget.Thread');
        ...
        var AbstractThreadWindow = Widget.extend({
            template: 'chatbot.legacy.mail.AbstractThreadWindow',
            custom_events: {
             ...
             * @abstract
             * @param {Widget} parent
             * @param {chatbot.legacy.mail.model.AbstractThread} [thread=null] the thread that this
             *   thread window is linked to. If not set, it is the "blank" thread
             *   window.
             * @param {Object} [options={}]
             * @param {chatbot.legacy.mail.model.AbstractThread} [options.thread]
             */
            ...
            renderHeader: function () {
                ...
                this.$header.html(
                    QWeb.render('chatbot.legacy.mail.AbstractThreadWindow.HeaderContent', options));
            },
            /**
            ...
            _onKeydown: function (ev) {
                ...
                // ENTER key (avoid requiring jquery ui for external chatbot)
                if (ev.which === 13) {
    ...
    odoo.define('chatbot.legacy.mail.model.CCThrottleFunctionObject', function (require) {
        "use strict";
    ...
    odoo.define('chatbot.legacy.mail.model.CCThrottleFunction', function (require) {
        "use strict";

        var CCThrottleFunctionObject = require('chatbot.legacy.mail.model.CCThrottleFunctionObject');
    ...
    odoo.define('chatbot.legacy.mail.model.Timer', function (require) {
        "use strict";
    ...
    odoo.define('chatbot.legacy.mail.model.Timers', function (require) {
        "use strict";
    ...
    odoo.define('chatbot.legacy.mail.widget.Thread', function (require) {
        "use strict";

        var DocumentViewer = require('chatbot.legacy.mail.DocumentViewer');
        var mailUtils = require('@mail/js/utils');
        ...
        var ThreadWidget = Widget.extend({
            ...
            /**
             * @param {chatbot.legacy.mail.model.AbstractThread} thread the thread to render.
             * @param {Object} [options]
            ...
            render: function (thread, options) {
                ...
                this.$el.html(QWeb.render('chatbot.legacy.mail.widget.Thread', {
                    thread: thread,
            ...
            renderLoading: function () {
                this.$el.html(QWeb.render('chatbot.legacy.mail.widget.ThreadLoading'));
            },
             ...
             *
             * @private
             * @param {chatbot.legacy.mail.model.AbstractMessage[]} messages list of messages in the
            ...
            _renderMessageNotificationPopover(messages) {
                ...
                this._messageMailPopover = this.$('.o_thread_tooltip').popover({
                    ...
                    content: function () {
                        ...
                        return QWeb.render('chatbot.legacy.mail.widget.Thread.Message.MailTooltip', {
             ...
             *
             * @private
             * @param {chatbot.legacy.mail.model.AbstractThread} thread with thread seen mixin,
             *   @see {chatbot.legacy.mail.model.ThreadSeenMixin}
             * @param {chatbot.legacy.mail.model.Message[]} messages list of messages in the
             *   rendered thread.
             */
            _renderMessageSeenPopover: function (thread, messages) {
                ...
                this._messageSeenPopover = this.$('.o_thread_message_core .o_mail_thread_message_seen_icon').popover({
                    ...
                    content: function () {
                        ...
                        return QWeb.render('chatbot.legacy.mail.widget.Thread.Message.SeenIconPopoverContent', {
                            thread: thread,
                            message: message,
                        });
                    },
    ...
    odoo.define('chatbot.legacy.mail.DocumentViewer', function (require) {
        "use strict";
        ...
        var DocumentViewer = Widget.extend({
            template: "chatbot.legacy.mail.DocumentViewer",
            events: {
            ...
            _updateContent: function () {
                this.$('.o_viewer_content').html(QWeb.render('chatbot.legacy.mail.DocumentViewer.Content', {
                    widget: this
                }));
            ...
            _onPrint: function (e) {
                ...
                var script = QWeb.render('chatbot.legacy.mail.PrintImage', {
                    src: src
                });
