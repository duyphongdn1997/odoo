...
registerClassPatchModel('mail.thread', 'chatbot/static/src/models/thread/thread.js', {
    ...
    convertData(data) {
        ...
        if ('chatbot_visitor' in data && data.chatbot_visitor) {
            if (!data2.members) {
                data2.members = [];
            }
            // `chatbot_visitor` without `id` is the anonymous visitor.
            if (!data.chatbot_visitor.id) {
                /**
                 ...
                 * record for every chatbot, whereas every correspondent should
                ...
                const partner = this.messaging.models['mail.partner'].create(
                    Object.assign(
                        this.messaging.models['mail.partner'].convertData(data.chatbot_visitor),
                ...
                const partnerData = this.messaging.models['mail.partner'].convertData(data.chatbot_visitor);
...
registerInstancePatchModel('mail.thread', 'chatbot/static/src/models/thread/thread.js', {
    ...
    _computeCorrespondent() {
        if (this.channel_type === 'chatbot') {
            // chatbot correspondent never change: always the public member.
            return;
        }
        return this._super();
    },
     ...
     */
    _computeDisplayName() {
        if (this.channel_type === 'chatbot' && this.correspondent) {
    ...
    _computeIsChatChannel() {
        return this.channel_type === 'chatbot' || this._super();
    },
    /**
     * @override
     */
    _getDiscussSidebarCategory() {
        switch (this.channel_type) {
            case 'chatbot':
                return this.messaging.discuss.categoryChatbot;
        }
        return this._super();
    }
});
