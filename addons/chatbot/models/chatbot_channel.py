...
class ChatbotChannel(models.Model):
    ...
    _name = 'chatbot.channel'
    _description = 'Chatbot Channel'

    # _rating_satisfaction_days = 7  # include only last 7 days to compute satisfaction

    def _default_image(self):
        image_path = modules.get_module_resource('chatbot', 'static/src/img', 'default.png')
    ...
    button_text = fields.Char('Text of the Button', default='Have a Question? Chat with us.',
                              help="Default text displayed on the Chatbot Support Button")
    default_message = fields.Char('Welcome Message', default='How may I help you?',
                                  help="This is an automated 'welcome' message that "
                                       "your visitor will see when they initiate a new conversation.")
    ...
    button_background_color = fields.Char(default="#878787", help="Default background color of the Chatbot button")
    button_text_color = fields.Char(default="#FFFFFF", help="Default text color of the Chatbot button")
    ...
    # relationnal fields
    user_ids = fields.Many2many('res.users', 'chatbot_channel_im_user', 'channel_id', 'user_id', string='Operators',
                                default=_default_user_ids)
    channel_ids = fields.One2many('mail.channel', 'chatbot_channel_id', 'Sessions')
    rule_ids = fields.One2many('chatbot.channel.rule', 'channel_id', 'Rules')
    ...
    def _compute_script_external(self):
        view = self.env.ref('chatbot.external_loader')
        values = {
    ...
    def _compute_web_page_link(self):
        for record in self:
            record.web_page = "%s/chatbot/%i" % (record.get_base_url(), record.id) if record.id else False

    @api.depends('channel_ids')
    def _compute_nbr_channel(self):
        data = self.env['mail.channel'].read_group([
            ('chatbot_channel_id', 'in', self._ids),
            ('has_message', '=', True)], ['__count'], ['chatbot_channel_id'], lazy=False)
        channel_count = {x['chatbot_channel_id'][0]: x['__count'] for x in data}
        for record in self:
    ...
    def _get_available_users(self):
        """ get available user of a given channel
            :returns : return the res.users having their im_status online
        """
        self.ensure_one()
        return self.user_ids.filtered(lambda user: user.im_status == 'online')

    def _get_chatbot_mail_channel_vals(self, anonymous_name, operator, user_id=None, country_id=None):
        ...
        return {
            ...
            'chatbot_channel_id': self.id,
            'anonymous_name': False if user_id else anonymous_name,
            'country_id': country_id,
            'channel_type': 'chatbot',
            'name': ' '.join([visitor_user.display_name if visitor_user else anonymous_name,
                              operator.chatbot_username if operator.chatbot_username else operator.name]),
            'public': 'private',
        }

    def _open_chatbot_mail_channel(self, anonymous_name, previous_operator_id=None, user_id=None, country_id=None):
        """ Return a mail.channel given a chatbot channel. It creates one with a connected operator,
        or return false otherwise
            ...
            :param country_id : the country of the anonymous person of the channel
            :type anonymous_name : str
            :return : channel header
            :rtype : dict

            If this visitor already had an operator within the last 7 days
            (information stored with the 'im_chatbot_previous_operator_pid' cookie),
        ...
        if previous_operator_id:
            ...
            if previous_operator_id in available_users.mapped('partner_id').ids:
                operator = next(available_user for available_user in available_users if
                                available_user.partner_id.id == previous_operator_id)
        if not operator:
        ...
        mail_channel_vals = self._get_chatbot_mail_channel_vals(anonymous_name, operator, user_id=user_id,
                                                                 country_id=country_id)
    ...
    def _get_random_operator(self):
        """ Return a random operator from the available users
        of the channel that have the lowest number of active chatbots.
        A chatbot is considered 'active' if it has at least one message within the 30 minutes.
        ...
        self.env.cr.execute("""SELECT COUNT(DISTINCT c.id), c.chatbot_operator_id
            FROM mail_channel c
            LEFT OUTER JOIN mail_message m ON c.id = m.res_id AND m.model = 'mail.channel'
            WHERE c.channel_type = 'chatbot'
            AND c.chatbot_operator_id in %s
            AND m.create_date > ((now() at time zone 'UTC') - interval '30 minutes')
            GROUP BY c.chatbot_operator_id
        ...
        active_channel_operator_ids = [active_channel['chatbot_operator_id'] for active_channel in active_channels]
        inactive_operators = [operator for operator in operators if
                              operator.partner_id.id not in active_channel_operator_ids]
        ...
        less_active_operator = random.choice([
            active_channel['chatbot_operator_id'] for active_channel in active_channels
    ...
    def get_chatbot_info(self, username=None):
        self.ensure_one()

        if username is None:
            username = 'Visitor'
        info = {'available': len(self._get_available_users()) > 0, 'server_url': self.get_base_url()}
        if info['available']:
...
class ChatbotChannelRule(models.Model):
    """ Channel Rules
        Rules defining access to the channel (countries, and url matching). It also provide the 'auto pop'
        option to open automatically the conversation.
    """

    _name = 'chatbot.channel.rule'
    _description = 'Chatbot Channel Rules'
    _order = 'sequence asc'
    ...
    auto_popup_timer = fields.Integer('Auto popup timer', default=0,
                                      help="Delay (in seconds) to automatically open the conversation window. "
                                           "Note: the selected action must be 'Auto popup' otherwise "
                                           "this parameter will not be taken into account.")
    channel_id = fields.Many2one('chatbot.channel', 'Channel',
                                 help="The channel of the rule")
    country_ids = fields.Many2many('res.country', 'chatbot_channel_country_rel', 'channel_id', 'country_id',
                                   'Country',
                                   help="The rule will only be applied for these countries. "
                                        "Example: if you select 'Belgium' and 'United States' "
                                        "and that you set the action to 'Hide Button', "
                                        "the chat button will be hidden on the specified "
                                        "URL from the visitors located in these 2 countries. "
                                        "This feature requires GeoIP installed on your server.")
    sequence = fields.Integer('Matching order', default=10,
                              help="Given the order to find a matching rule. If 2 rules are matching for the given "
                                   "url/country, the one with the lowest sequence will be chosen.")
