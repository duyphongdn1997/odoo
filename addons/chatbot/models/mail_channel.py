...
class MailChannel(models.Model):
    ...
    channel_type = fields.Selection(selection_add=[('chatbot', 'Chatbot Conversation')])
    chatbot_active = fields.Boolean('Is chatbot ongoing?',
                                    help='Chatbot session is active until visitor leave the conversation.')
    chatbot_channel_id = fields.Many2one('chatbot.channel', 'Channel')
    chatbot_operator_id = fields.Many2one('res.partner', string='Operator', help="Operator for this specific channel")
    country_id = fields.Many2one('res.country', string="Country", help="Country of the visitor of the channel")

    _sql_constraints = [('chatbot_operator_id',
                         "CHECK((channel_type = 'chatbot' "
                         "and chatbot_operator_id is not null) or (channel_type != 'chatbot'))",
                         'Chatbot Operator ID is required for a channel of type chatbot.')]

    def _compute_is_chat(self):
        super(MailChannel, self)._compute_is_chat()
        for record in self:
            if record.channel_type == 'chatbot':
                record.is_chat = True

    def _channel_message_notifications(self, message, message_format=False):
        """ When a anonymous user create a mail. Channel, the operator is not notify (to avoid massive polling when
            clicking on chatbot button). So when the anonymous person is sending it's FIRST message, the channel header
            should be added to the notification, since the user cannot be listening to the channel.
        """
        chatbot_channels = self.filtered(lambda x: x.channel_type == 'chatbot')
        other_channels = self.filtered(lambda x: x.channel_type != 'chatbot')
        notifications = super(MailChannel,
                              chatbot_channels)._channel_message_notifications(
            message.with_context(chatbot_use_username=True)) + \
                        super(MailChannel,
                              other_channels)._channel_message_notifications(message, message_format)
        for channel in self:
            # add uuid for private chatbot channels to allow anonymous to listen
            if channel.channel_type == 'chatbot' and channel.public == 'private':
    ...
    def _channel_fetch_message(self, last_id=False, limit=20):
        """ Override to add the context of the chatbot username."""
        channel = self.with_context(chatbot_use_username=True) if self.channel_type == 'chatbot' else self
        return super(MailChannel, channel)._channel_fetch_message(last_id=last_id, limit=limit)

    def channel_info(self):
        """ Extends the channel header by adding the chatbot operator and the 'anonymous' profile
            :rtype : list(dict)
        """
        ...
        for channel in self:
            ...
            if channel.channel_type == 'chatbot':
                ...
                if channel.chatbot_operator_id:
                    res = channel.chatbot_operator_id.with_context(chatbot_use_username=True).name_get()[0]
                    channel_infos_dict[channel.id]['operator_pid'] = (res[0], res[1].replace(',', ''))
                # add the anonymous or partner name
                channel_infos_dict[channel.id]['chatbot_visitor'] = channel._channel_get_chatbot_visitor_info()
        return list(channel_infos_dict.values())

    def _channel_get_chatbot_visitor_info(self):
        self.ensure_one()
        ...
        partners = channel_partner_ids - self.chatbot_operator_id
        if not partners:
        ...
        return {
            ...
            'id': False,
            'name': self.anonymous_name or "Visitor",
        }

    def _channel_get_chatbot_partner_name(self):
        if self.chatbot_operator_id in self.channel_partner_ids:
            partners = self.channel_partner_ids - self.chatbot_operator_id
            if partners:
                partner_name = False
    ...
    @api.autovacuum
    def _gc_empty_chatbot_sessions(self):
        ...
        self.env.cr.execute("""
            ...
            ) AND C.channel_type = 'chatbot' AND chatbot_channel_id IS NOT NULL AND
    ...
    def _execute_command_help_message_extra(self):
        ...
        return msg + "Type <b>:shortcut</b> to insert a canned response in your message.<br>"

    def execute_command_history(self, **kwargs):
        self.env['bus.bus']._sendone(self.uuid, 'chatbot.history_command', {'id': self.id})

    def _send_history_message(self, pid, page_history):
        message_body = 'No history found'
        if page_history:
    ...
    def _close_chatbot_session(self, **kwargs):
        """ Set deactivate the chatbot channel and notify (the operator) the reason of closing the session."""
        self.ensure_one()
    ...
    def _email_chatbot_transcript(self, email):
        company = self.env.user.company_id
        render_context = {
            "company": company,
            "channel": self,
        }
        template = self.env.ref('chatbot.chatbot_email_template')
        mail_body = template._render(render_context, engine='ir.qweb', minimal_qcontext=True)
        mail_body = self.env['mail.render.mixin']._replace_local_links(mail_body)
        mail = self.env['mail.mail'].sudo().create({
            'subject': f'Conversation with {self.chatbot_operator_id.name}',
