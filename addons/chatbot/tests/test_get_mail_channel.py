...
class TestGetMailChannel(TransactionCase):
    def setUp(self):
        ...
        self.chatbot_channel = self.env['chatbot.channel'].create({
            'name': 'The channel',
        ...
        self.patch(type(self.env['chatbot.channel']), '_get_available_users', get_available_users)

    def test_get_mail_channel(self):
        """For a chatbot with 5 available operators, we open 5 channels 5 times (25 channels total).
        For every 5 channels opening, we check that all operators were assigned.
        """

        for i in range(5):
            mail_channels = self._open_chatbot_mail_channel()
    ...
    def test_channel_get_chatbot_visitor_info(self):
        ...
        channel_info = self.chatbot_channel.with_user(public_user)._open_chatbot_mail_channel(anonymous_name='Visitor 22', country_id=belgium.id)
        visitor_info = channel_info['chatbot_visitor']
        ...
        channel_info = self.chatbot_channel.with_user(test_user)._open_chatbot_mail_channel(anonymous_name='whatever', user_id=test_user.id)
        visitor_info = channel_info['chatbot_visitor']
        ...
        channel_info = self.chatbot_channel.with_user(operator)._open_chatbot_mail_channel(anonymous_name='whatever', previous_operator_id=operator.partner_id.id, user_id=operator.id)
    ...
    def _open_chatbot_mail_channel(self):
        mail_channels = []

        for i in range(5):
            mail_channel = self.chatbot_channel._open_chatbot_mail_channel('Anonymous')
    ...
    def test_channel_not_pinned_for_operator_before_first_message(self):
        public_user = self.env.ref('base.public_user')
        channel_info = self.chatbot_channel.with_user(public_user)._open_chatbot_mail_channel(anonymous_name='whatever')
